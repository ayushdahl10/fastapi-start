...
def registered_user_permissions() -> List:
    return {
    ...
    for role in roles:
        ...
        for permission in permissions:
            for p in permission:
                ...
                role_perm_exists = check_permission(
                    db=db,
                    role_id=role_instance.id,
                    permission_id=permission_instance.id,
                )
                if role_perm_exists == False:
                    permission_id.append(permission_instance.uid)
        if permission_id:
            update_roles_with_permissions(
                db=db, permissions=permission_id, role_name=role.get("name")
            )
