...
@user_permission
def create_role(
    request: Request, role: schemas.CreateRole, db: Session = Depends(get_db)
):
    check_role_exists = crud.get_role_by_name(db, role.name)
    if check_role_exists:
        raise HTTPException(
            detail={"error": f"role already exists with name {role.name}"},
            status_code=400,
        )
...
def get_role_by_id(request: Request, role_id: str, db: Session = Depends(get_db)):
    ...
    if not role:
        raise HTTPException(
            detail={"error": f"role not found with id= {role_id}"},
...
def update_role_by_id(
    request: Request,
    role_id: str,
    role: schemas.CreateRolePermissions,
