...
async def __check_permissions(request: Request, user: models.User, name: str) -> bool:
    ...
    for role in user_roles:
        permission_id = perm_crud.get_permission_by_name_and_method(
            db=db, name=name, method=method
        )
        if not permission_id:
            raise forbitten_message
        has_permission = perm_crud.check_permission(
            db=db, role_id=role.id, permission_id=permission_id.id
        )
    if has_permission:
        return True
    else:
