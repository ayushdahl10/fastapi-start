...
def create_web_config(db: Session, web_config: WebConfigCreate):
    web_config_obj = models.WebConfig(
        uid=shortuuid.uuid(),
        key=web_config.key,
        value=web_config.value,
...
def read_all_config(db: Session, skip: int, limit: int):
    return (
        db.query(models.WebConfig)
        .filter(not models.WebConfig.is_deleted)
        .offset(skip)
        .limit(limit)
    )
...
def get_config_by_id(db: Session, uid: str):
    return (
        db.query(models.WebConfig)
        .filter(models.WebConfig.uid == uid, not models.WebConfig.is_deleted)
        .first()
    )
