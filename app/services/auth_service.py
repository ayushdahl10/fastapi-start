...
def get_user(db:Session, user_id:str):
    instance= db.query(models.User).options(joinedload(models.User.role)).filter(models.User.uid==user_id).first()
    return instance

def create_user(db: Session, user: schemas.UserCreate):
    user.password=hash_password(user.password)
    db_user =models.User(email=user.email, password=user.password, uid=shortuuid.uuid(),first_name=user.first_name,last_name=user.last_name,age=user.age,username=user.email)
    db.add(db_user)
    db.commit()
    db.refresh(db_user)
    return db_user
...
def create_user_admin(db:Session,user:schemas.UserAdmin):
    ...
    if roles:
        ...
        raise HTTPException(detail={"error":"roles field cannot be empty"},status_code=400)
    db.add(db_user)
    db.commit()
    db.refresh(db_user)
...
def get_users(db:Session,skip:int=0,limit:int=10):
    user= db.query(models.User).options(joinedload(models.User.role)).filter(models.User.is_active==True).offset(skip).limit(limit)
    return user
