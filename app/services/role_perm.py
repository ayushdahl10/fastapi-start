...
def get_permission_by_url_and_method(db:Session,url_path:str,method:str):
    db_perm=db.query(models.Permissions).filter(models.Permissions.url_path==url_path,models.Permissions.method==method).first()
    return db_perm
...
def get_permission_by_name_and_method(db:Session,name:str,method:str):
    method=method.upper()
    db_perm=db.query(models.Permissions).filter(models.Permissions.name==name,models.Permissions.method==method).first()
    return db_perm
...
def get_role_by_id(db:Session,role_id:str):
    role_instance= db.query(models.Roles).options(joinedload(models.Roles.permission)).filter(models.Roles.uid==role_id).first()
...
def check_permission(db:Session,role_id:int,permission_id:int)->bool:
    has_perm= db.query(models.RolePermissions).filter(models.RolePermissions.permission==permission_id,models.RolePermissions.role==role_id).first()
    if not has_perm:
        return False
    return True
